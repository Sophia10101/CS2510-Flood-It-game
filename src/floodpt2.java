...
class FloodItWorld extends World {
  ...
  ArrayList<Cell> toBeFlooded = new ArrayList<>(); // Cells to be flooded in the next tick
  Color targetColor; // Target color for the current flood operation
  Color originalColor; // Original color of the cell to be changed
  ...
  public void onMouseClicked(Posn pos) {
    ...
    int offsetY = (sceneSize - size * cellSize) / 2 + 40; // Adjust y position to account for menu height
    ...
    if (pos.y < 40) {
      if (sizeDropdownOpen) {
        ...
        if (pos.x > offsetX + sceneSize / 2 - 200 && pos.x < offsetX + sceneSize / 2 - 150) {
          sizeDropdownOpen = true;
        } else if (pos.x > offsetX + sceneSize / 2 - 100 && pos.x < offsetX + sceneSize / 2 - 50) {
          colorsDropdownOpen = true;
        } else if (pos.x > offsetX + sceneSize / 2 - 40 && pos.x < offsetX + sceneSize / 2 + 40) {
    ...
    if (x >= 0 && x < size && y >= 0 && y < size) {
      ...
      if (!clickedCell.color.equals(board.get(0).color)) {
        steps++; // Increment steps only if a valid move changes the state
        originalColor = board.get(0).color; // Save the original color
        targetColor = clickedCell.color; // Set the target color to the clicked cell's color
        toBeFlooded.clear(); // Clear any previous cells to be flooded
        for (Cell cell : board) {
  ...
  int extractSize(String sizeString) {
    String xPart = sizeString.substring(0, sizeString.indexOf('x'));
    return stringToInt(xPart);
  }
  ...
  int convertDigitStringToInt(String digitStr) {
    ...
    if (digitStr.equals("1")) 
      return 1;
    if (digitStr.equals("2")) 
      return 2;
    if (digitStr.equals("3")) 
      return 3;
    if (digitStr.equals("4")) 
      return 4;
    if (digitStr.equals("5")) 
      return 5;
    if (digitStr.equals("6")) 
      return 6;
    if (digitStr.equals("7")) 
      return 7;
    if (digitStr.equals("8")) 
      return 8;
    if (digitStr.equals("9")) 
      return 9;
  ...
  public void onKeyEvent(String key) {
    if (key.equals("r")) {
      resetBoard(this.size); // Reset the board to the current size
    }
  }
  ...
  public WorldScene makeScene() {
    ...
    WorldImage background = new RectangleImage(sceneSize, sceneSize + 60, OutlineMode.SOLID, Color.WHITE);
    scene.placeImageXY(background, sceneSize / 2, (sceneSize + 60) / 2);


    scene.placeImageXY(new TextImage("Size: ", 15, Color.BLACK), offsetX + sceneSize / 2 - 250, 20);
    scene.placeImageXY(new RectangleImage(50, 20, OutlineMode.SOLID, Color.WHITE), offsetX + sceneSize / 2 - 200, 20);
    scene.placeImageXY(new TextImage(sizes.get(selectedSizeIndex), 15, Color.BLACK), offsetX + sceneSize / 2 - 200, 20);
    ...
    if (sizeDropdownOpen) {
      for (int i = 0; i < sizes.size(); i++) {
        ...
        scene.placeImageXY(new RectangleImage(50, 20, OutlineMode.SOLID, color), offsetX + sceneSize / 2 - 200, 40 + i * 20);
        scene.placeImageXY(new TextImage(sizes.get(i), 15, Color.BLACK), offsetX + sceneSize / 2 - 200, 40 + i * 20);
      }
    }


    scene.placeImageXY(new TextImage("Colors: ", 15, Color.BLACK), offsetX + sceneSize / 2 - 140, 20);
    scene.placeImageXY(new RectangleImage(50, 20, OutlineMode.SOLID, Color.WHITE), offsetX + sceneSize / 2 - 100, 20);
    scene.placeImageXY(new TextImage(numColorsOptions.get(selectedNumColorsIndex), 15, Color.BLACK), offsetX + sceneSize / 2 - 100, 20);
    ...
    if (colorsDropdownOpen) {
      for (int i = 0; i < numColorsOptions.size(); i++) {
        ...
        scene.placeImageXY(new RectangleImage(50, 20, OutlineMode.SOLID, color), offsetX + sceneSize / 2 - 100, 40 + i * 20);
        scene.placeImageXY(new TextImage(numColorsOptions.get(i), 15, Color.BLACK), offsetX + sceneSize / 2 - 100, 40 + i * 20);
      }
    }


    scene.placeImageXY(new RectangleImage(80, 30, OutlineMode.SOLID, Color.GREEN), offsetX + sceneSize / 2 - 40, 20);
    scene.placeImageXY(new TextImage("New Game", 15, Color.BLACK), offsetX + sceneSize / 2 - 40, 20);


    int outlineWidth = size * cellSize + 10;
    int outlineHeight = size * cellSize + 10; 
    WorldImage outline = new RectangleImage(outlineWidth, outlineHeight, OutlineMode.SOLID, Color.BLACK);
    scene.placeImageXY(outline, offsetX + size * cellSize / 2, offsetY + size * cellSize / 2);
  ...
  public void onTick() {
    ...
    for (Cell cell : toBeFlooded) {
      ...
      if (cell.left != null && !cell.left.flooded && cell.left.color.equals(originalColor)) {
        nextToBeFlooded.add(cell.left);
      }
      if (cell.right != null && !cell.right.flooded && cell.right.color.equals(originalColor)) {
        nextToBeFlooded.add(cell.right);
      }
      if (cell.top != null && !cell.top.flooded && cell.top.color.equals(originalColor)) {
        nextToBeFlooded.add(cell.top);
      }
      if (cell.bottom != null && !cell.bottom.flooded && cell.bottom.color.equals(originalColor)) {
  ...
  public WorldImage createSmileyFace(int radius) {
    ...
    int mouthOffsetY = -radius / 2; // Move the mouth up by reversing the offset
  ...
  public WorldScene makeEndScene(String message) {
    ...
    if (message.equals("You Won!" + " In: " + steps + " tries!")) {
...
class ExamplesFloodItWorld {
  ...
  FloodItWorld world = new FloodItWorld(10, 2);
  ...
  void testMakeEndScene(Tester t) {
    ...
    expectedWinScene.placeImageXY(new TextImage("You Won!", 32, FontStyle.BOLD, Color.MAGENTA), 200, 100);
    ...
    expectedLoseScene.placeImageXY(new TextImage("You Lost :(", 32, FontStyle.BOLD, Color.MAGENTA), 200, 100);
  ...
  void testOnKeyEvent(Tester t) {
    ...
    // Simulate pressing the "r" key to reset the board
    world.onKeyEvent("r");
